...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= totalDays; day++) {
            ...
            if (yokozawaHoliday) {
                ...
                b.classList.add('day-band', 'band-holiday');
                b.textContent = '横澤 休日';
            ...
            if (yokozawaIn) {
                ...
                b.textContent = `横澤 出勤 ${formatTime(yokozawaIn.datetime)}`;
                bandList.appendChild(b);
            }
            // 3. 横澤 退勤
            if (yokozawaOut) {
                const b = document.createElement('div');
                b.classList.add('day-band', 'band-yokozawa');
                b.textContent = `横澤 退勤 ${formatTime(yokozawaOut.datetime)}`;
                bandList.appendChild(b);
            }
            // 4. 鈴木 出勤
            if (suzukiIn) {
                ...
                b.textContent = `鈴木 出勤 ${formatTime(suzukiIn.datetime)}`;
                bandList.appendChild(b);
            }
            // 5. 鈴木 退勤
            if (suzukiOut) {
                const b = document.createElement('div');
                b.classList.add('day-band', 'band-suzuki');
                b.textContent = `鈴木 退勤 ${formatTime(suzukiOut.datetime)}`;
                bandList.appendChild(b);
            }
    ...
    async function checkAndRunAutoMode() {
        ...
        for (const user of users) {
            ...
            // 退勤の自動打刻 (出勤が既にある、または自動打刻済みの状態で退勤を打刻)
            if ((hasIn || localStorage.getItem(`autoDone_${user}_${todayStr}_出勤`)) && !hasOut) {
                if (currentTimeStr >= autoTime.out) {
